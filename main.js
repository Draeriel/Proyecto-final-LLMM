        ...
        $.each( data[counter], function( key, val ) {
            ...
            if (key == "text") {
                for (var frase = 0; frase < data[counter].text.length; frase++){
                    $(parrafo).append(items.push("<p>" + data[counter].text[frase] + "</p>"))                    
                };
                console.log(parrafo);
                items.push("<div class='modal fade' id='" + id + "' role='dialog'> <div class='modal-dialog modal-lg'><div class='modal-content'><div class='modal-header'><h4 class='modal-title'>" + title + "</h4></div><div class='modal-body'>" + parrafo + "</div><div class='modal-footer'><button type='button' class='btn btn-default' data-dismiss='modal'>Close</button></div></div></div></div>");
                parrafo = []
            }
            if (key == "date") {
